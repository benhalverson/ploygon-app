...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("jarhead name service", "dd214")
    {
        tld = _tld;
    ...
    function register(string calldata name) public payable {
        ...
        // combine the name passed into the fuction with the TLD
        string memory _name = string(abi.encodePacked(name, ".", tld));
        string memory finalSvg = string(
            abi.encodePacked(svgOne, _name, svgTwo)
        );
        ...
        console.log(
            "Registering %s. %s on th econtract with tokendId %d",
            name,
            tld,
            newRecordId
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Jarhead name service", "image": "data:image/svg+xml;base64,',
        ...
        console.log("Token URI: %s", finalTokenUri);
